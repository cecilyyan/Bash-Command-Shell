...
std::string myShell::replaceVar(std::string userInput) {
  ...
  std::cout << "before dollarsplit" << std::endl;
  std::vector<std::string> toReplace = dollarSplit(userInput);
  ...
  while (it != toReplace.end()) {
    ...
    mapSearch = varMap.find(*it);
    if (mapSearch != varMap.end()) {
      //find the variable, then replace &var with its value
      std::cout << "find the variable in map to replace" << std::endl;
      replaceInput.append(mapSearch->second);
      std::cout << "print out the toReplace " << replaceInput << std::endl;
    }
    else {
      //does not find, no need to replace
      std::cout << "no need to replace" << std::endl;
      replaceInput.append(*it);
