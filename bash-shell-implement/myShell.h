...
class myShell
{
 ...
 public:
  ...
  //set input to command and argument string and update first_command and argumentStr
  ...
  /*replace the variable when dollar sign is the first char of userinput
print out the var value and the special characters to std::cout
*/
  void printVar(std::string userInput);
...
void myShell::printVar(std::string userInput) {
  std::string output;
  std::vector<std::string> varVect;
  varVect = dollarSplit(userInput);
  std::vector<std::string>::iterator it = varVect.begin();
  while (it != varVect.end()) {
    //use iterator to search whether in varMap exist the variable
    std::map<std::string, std::string>::iterator mapSearch;
    bool exit = false;

    for (size_t i = 0; i < (*it).size(); i++) {
      std::string toSearch = (*it).substr(0, (*it).size() - i);

      mapSearch = varMap.find(toSearch);
      if (mapSearch != varMap.end()) {
        //find the variable in varMap so replace it with its value
        exit = true;
        output = output + varMap[toSearch] + (*it).substr((*it).size() - i);

        break;
      }
    }
    if (exit == false) {
      std::cout << "variable " << (*it) << " does not exit" << std::endl;
    }
...
std::string myShell::replaceVar(std::string userInput) {
  ...
  std::vector<std::string> toReplace = dollarSplit(userInput.substr(userInput.find("$")));
  if (toReplace[1] == userInput) {
    return userInput;
  }
  ...
  while (it != toReplace.end()) {
    ...
    for (size_t i = 0; i < (*it).size(); i++) {
      ...
      if (mapSearch != varMap.end()) {
        //find the variable, then replace &var with its value
        exit = true;
        replaceInput.append(mapSearch->second);
        replaceInput.append((*it).substr((*it).size() - i));
        break;
      }
    }
    if (exit == false) {
  ...
  replaceInput = command + replaceInput;
  std::cout << replaceInput << std::endl;
  return replaceInput;
}
void myShell::parse(std::string user_command) {
  std::string userCommand = user_command;
  size_t size = userCommand.size();  //size of usercommand
  //find the first white space and before that is the command
  int first_blank_index = userCommand.find(" ");
  //if the first white space position is userCommand[0], then delete the white space
  size_t blankToDelete = 0;  //number of white space needed to delete
  while (first_blank_index == 0) {
    blankToDelete++;
    userCommand = userCommand.substr(1, size - blankToDelete);
    first_blank_index = userCommand.find(" ");
  }
...
void myShell::execuate(char * argv) {
  ...
  if (curr_pid == 0) {
    ...
    for (size_t i = 0; i < size; i++) {
      std::string one_argument = arguments[i];
      oneArgument = strdup(one_argument.c_str());
      argument_list[i + 1] = oneArgument;
    }
...
std::string myShell::getPath(std::string argv) {
  ...
  //if the std::cin contains / then return std::cin
  ...
  if (command.find("/") != std::string::npos) {
    ...
    if (it == path.end()) {
      std::cout << "Command " << argv << " not found" << std::endl;
      command = "";
    }
  }
  return command;
...
void myShell::cdCommand(std::string directory) {
  int res = chdir(directory.c_str());
  std::cout << directory << std::endl;
  if (res == -1) {
    perror("cd error: directory does not exit\n");
  }
}
